...
for i, message in enumerate(messages):
    ...
    try:
        ...
        formatted_date = datetime.strptime(date_time, "%B %d, %Y %I:%M:%S %p").strftime("%B %d, %Y")
        formatted_time = datetime.strptime(date_time, "%B %d, %Y %I:%M:%S %p").strftime("%I:%M %p")
        print(f'{i+1}. Subject "{subject}", Date: "{formatted_date}", Time: "{formatted_time}"')
        time.sleep(2)  # add a 2-second delay after each request
    except HttpError as error:
        print(f'Error retrieving details for message with ID {msg_id}: {error}')
...
for message in messages:
    ...
    print(f'Subject "{subject}", Date: "{datetime.strptime(date_time, "%B %d, %Y %I:%M:%S %p").strftime("%B %d, %Y")}", Time: "{datetime.strptime(date_time, "%B %d, %Y %I:%M:%S %p").strftime("%I:%M %p")}" was successfully deleted.')
